# Guía de despliegue — Netlify + MongoDB Atlas

Documento que junta toda la información necesaria para colgar el portfolio en **Netlify** usando **MongoDB Atlas** como base de datos en la nube.
...
| --- | --- | --- |
| El código/sitio (HTML, React, rutas) | `localhost:3000` | **Netlify** (servidores) |
| Los datos (proyectos del portfolio) | MongoDB en tu PC | **MongoDB Atlas** (nube) |
| Tus secretos (`ADMIN_PIN`, URI) | `.env.local` | Variables de entorno en Netlify |

En desarrollo los 3 están juntos en tu PC. Al desplegar se separan, y cada uno cumple su rol.

> **¿Necesito `localhost:3000`?**
> No. `localhost:3000` es solo tu máquina, para desarrollar. En Netlify el sitio corre en los servidores de ellos: compilan el proyecto (`npm run build`), lo hostean y te dan una URL pública (ej. `tu-sitio.netlify.app`). No tenés que dejar nada abierto en tu PC.

> **¿Y la base de datos?**
> El MongoDB local (`mongodb://localhost:27017`) no funciona en producción: Netlify no puede conectarse a tu computadora. Necesitás una base en la nube → **MongoDB Atlas** (plan free, 512 MB, alcanza de sobra para un portfolio).
...
- Tu base de datos **no vive en tu PC** — vive en los servidores de Atlas, prendidos 24/7. Así Netlify puede leerla aunque tu computadora esté apagada.
...
| **IP `0.0.0.0/0`** | Permitir conexiones desde cualquier IP del mundo (necesario porque Netlify se conecta desde servidores variables). |
| **GitHub** | El depósito del código fuente. Netlify descarga el código desde acá. |
| **Importar en Netlify** | Netlify descarga tu repo, compila (`npm run build`) y sirve el resultado en una URL pública. |
| **Variables de entorno** | Secretos guardados en el servidor de Netlify. Reemplazan a tu `.env.local` en producción. |
...
3. Build command: `npm run build` — Publish directory: `.next` (Netlify detecta Next.js solo).
4. Deploy.
...
1. Entrá a tu `.netlify.app` y verificá que carguen los proyectos.
...
- Las API routes (`/api/admin/*`) corren como **serverless functions** de Netlify: funcionan igual que en local, se encienden bajo demanda.
- Cada función serverless se conecta a MongoDB; `lib/mongodb.js` ya cachea la conexión con `globalThis`, así que está bien.
- Cada vez que hacés `git push` al repo, Netlify **redeployea solo**.
- Si querés tu propio dominio (ej. `lucio.com.ar`), se configura en **Domain management** de Netlify.
...
Vos (creás proyectos en /admin)
   │  (HTTPS)
   ▼
Netlify ──(serverless function)──► MongoDB Atlas
   ▲                                  │
   │                                  ▼
Vos mirás el sitio en tu netlify.app  Datos guardados 24/7
```
